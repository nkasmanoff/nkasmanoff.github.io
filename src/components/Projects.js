...
const projects = [
  ...
  },
  {
    ...
    tech: ['LLM fine-tuning', 'Text to speech', 'Speech to text', 'Multi-modal AI', 'Agentic AI'],
  ...
  {
    title: 'MLX Framework',
    description: 'Implemented LLaVA, a vision language model, in MLX, a framework for building AI applications for Apple devices.',
...
const Projects = () => {
  return (
    <section id="projects" className="py-20">
      <h2 className="text-3xl font-bold mb-8 text-center text-slate-800 dark:text-cyan-400">Projects</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project, index) => (
          <a href={project.link} key={index} className="transition-transform hover:scale-105 no-underline" target="_blank" rel="noopener noreferrer">
            <Card className="bg-white/50 dark:bg-slate-800/50 border-slate-200 dark:border-cyan-500/20 backdrop-blur-sm h-full">
              ...
              <CardHeader>
                <CardTitle className="text-slate-800 dark:text-cyan-400">{project.title}</CardTitle>
                <CardDescription className="text-slate-600 dark:text-gray-300">{project.description}</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {project.tech.map((tech, techIndex) => (
                    <span key={techIndex} className="bg-blue-50 text-blue-700 dark:bg-cyan-900/50 dark:text-cyan-300 px-2 py-1 rounded-full text-sm dark:border dark:border-cyan-500/20">
                      {tech}
                    </span>
                  ))}
                </div>
